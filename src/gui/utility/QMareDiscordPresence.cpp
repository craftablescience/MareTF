...
QMareDiscordPresence::ButtonData g_DiscordButton1;
QMareDiscordPresence::ButtonData g_DiscordButton2;

void QMareDiscordPresence::init(std::string_view appID) {
...
void QMareDiscordPresence::setState(std::string state_) {
...
void QMareDiscordPresence::setDetails(std::string details_) {
...
void QMareDiscordPresence::setLargeImage(std::string imageKey) {
...
void QMareDiscordPresence::setLargeImageText(std::string text) {
...
void QMareDiscordPresence::setSmallImage(std::string imageKey) {
	if (imageKey.length() <= MAXIMUM_IMAGE_KEY_LENGTH) {
...
void QMareDiscordPresence::setSmallImageText(std::string text) {
	if (text.length() <= MAXIMUM_STRING_LENGTH) {
...
void QMareDiscordPresence::setStartTimestamp(std::int64_t time) {
	g_DiscordStartTimestamp = time;
	g_DiscordDirty = true;
}

void QMareDiscordPresence::setEndTimestamp(std::int64_t time) {
	g_DiscordEndTimestamp = time;
	g_DiscordDirty = true;
}

void QMareDiscordPresence::setTopButton(ButtonData button) {
	g_DiscordButton1 = std::move(button);
	g_DiscordDirty = true;
}

void QMareDiscordPresence::setBottomButton(ButtonData button) {
	g_DiscordButton2 = std::move(button);
	g_DiscordDirty = true;
}

void QMareDiscordPresence::update() {
	if (!g_DiscordInitialized) {
		return;
	}
	if (g_DiscordDirty) {
...
void QMareDiscordPresence::reset() {
	g_DiscordState = "";
	g_DiscordDetails = "";
	g_DiscordLargeImage = "";
...
void QMareDiscordPresence::shutdown() {
	if (g_DiscordInitialized) {
		Discord_Shutdown();

...
if(MARETF_BUILD_GUI OR MARETF_BUILD_THUMBNAILER)
    ...
    if(WIN32)
        ...
        foreach(QT_REQUIRED_ROOT_LIB ${QT_REQUIRED_ROOT_LIBS})
            if(EXISTS "${QT_BASEDIR}/bin/Qt6${QT_REQUIRED_ROOT_LIB}${QT_LIB_SUFFIX}.dll")
                configure_file("${QT_BASEDIR}/bin/Qt6${QT_REQUIRED_ROOT_LIB}${QT_LIB_SUFFIX}.dll" "${CMAKE_BINARY_DIR}/Qt6${QT_REQUIRED_ROOT_LIB}${QT_LIB_SUFFIX}.dll" COPYONLY)
            endif()
        endforeach()
    ...
    elseif(UNIX AND DEFINED QT_BASEDIR)
        foreach(QT_REQUIRED_ROOT_LIB ${QT_REQUIRED_ROOT_LIBS})
            if(EXISTS "${QT_BASEDIR}/lib/libQt6${QT_REQUIRED_ROOT_LIB}.so.6")
                configure_file("${QT_BASEDIR}/lib/libQt6${QT_REQUIRED_ROOT_LIB}.so.6" "${CMAKE_BINARY_DIR}/libQt6${QT_REQUIRED_ROOT_LIB}.so.6" COPYONLY)
            endif()
        endforeach()
